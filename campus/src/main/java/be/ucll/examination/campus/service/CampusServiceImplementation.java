...
public class CampusServiceImplementation implements CampusService {
    ...
    @Override
    public Optional<Local> findLocalByCampusAndName(String campusName, String localName) {
        Campus campus = campusRepository.findCampusByName(campusName).orElseThrow(
                CampusNameDoesntExistException::new
        );

        return Optional.of(campus.getLocals()
                .stream()
                .filter(local -> local.getName().equals(localName))
                .findFirst()
                .orElseThrow(
                        LocalNameDoesntExistException::new
                ));
    }
